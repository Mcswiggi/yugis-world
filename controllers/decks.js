...
export { index, create, show, deleteCardFromDeck, newDeck as new }
...
function deleteCardFromDeck(req, res) {
  ...
  Card.findById(req.params.id).then((card) => {
    ...
    card.save().then(() => {
      Deck.findById(req.body.deckId).then((deck) => {
        deck.cards.remove(card._id)
        deck.save().then(() => {
          card, res.redirect(`/decks/${req.body.deckId}`)
        })
      })
    })
  })
...
function create(req, res) {
  ...
  Deck.create(req.body)
    .then((deck) => {
      Profile.findById(req.body.owner).then((profile) => {
